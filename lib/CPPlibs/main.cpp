...
char* cropped_rgb_global;
char* cropped_depth_global;
...
char* draw_rectangle(double angle, int rec_size, cv::Point joint_pt, cv::Mat input_image, cv::Mat source_image){
    ...
    resized_rgb_crop = cv::Mat(cv::Size(224,224), CV_32FC3);
...
char* draw_rectangle_depth(double angle, int rec_size, cv::Point joint_pt, cv::Mat input_image, cv::Mat source_image){
    ...
    resized_rgb_crop = cv::Mat(cv::Size(224,224), CV_8UC1);
    cv::resize(rgb_cropped, resized_rgb_crop, resized_rgb_crop.size(), 0, 0, CV_INTER_AREA);
...
extern "C"{
    ...
    // croppedResult* ccrop_rgb_depth(int height, int width, uchar* rgb_frame_data, uchar* depth_frame_data, int center_x, int center_y, int rec_size, double angle){
    //     cv::Point center_pt(center_x, center_y);
    //     Mat image(height, width, CV_8UC3, rgb_frame_data);
    //     Mat depth(height+2, width, CV_8UC1, depth_frame_data);
    //     // double angle, int rec_size, cv::Point joint_pt, cv::Mat input_image, cv::Mat source_image, cv::Mat depth_image
    //     auto tmp = crop_rgb_depth(angle, rec_size, center_pt, image, image, depth);
    //     if(tmp != nullptr)
    //         return tmp;
    // }
    ...
    char* cget_cropped_rgb(){
        if(cropped_rgb_global != nullptr)
        {
            char* tmp = cropped_rgb_global;
    ...
    char* cget_cropped_depth(){
        if(cropped_depth_global != nullptr)
        {
            char* tmp = cropped_depth_global;
