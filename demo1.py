...
from lib.utils.camera_wrapper import CustomDataset, VideoReader, VideoReaderWithDepth, CameraReader, CameraInfo
...
def run_demo_with_hand(model, refine_model, frame_provider, cfg, device):
    delay = 1
    esc_code = 27
    p_code = 112
    mean_time = 0
    ...
    # 手的检测框的点
    int_resPoints = ctypes.c_int * 8   # (x,y) * 2
    points_arr = int_resPoints()
    
    action_time_list = []
    total_time = 0.0
    while not rospy.is_shutdown():
        for (img, img_trans, scales, depth) in frame_provider:   # img, img_trans, scales, depth --> 单通道
            """
            img: 原图
            ...
            """
            # depth --> 3通道
            # depth_three_ch = cv2.merge([depth, depth, depth])
                ...
                if len(pred_bodys_2d) > 0:  # 除去不合理的人之后
                    ...
                    # ------------------------------------------------------------------------------
                        ...
                        for i in range(len(current_pose_list)):
                            ...
                            cv2.namedWindow(lhand_depth_name, cv2.WINDOW_NORMAL)
                            cv2.namedWindow(rhand_depth_name, cv2.WINDOW_NORMAL)
                            # --------------------------------------------------
                            ...
                            if human_id == 0:
                                if lelbow_y_3d > lwrist_y_3d and ori_resoulution_bodys_2D[i, 4, 1] <= (1080 * 3 / 4):
                                ...
                                if relbow_y_3d > rwrist_y_3d and ori_resoulution_bodys_2D[i, 11, 1] <= (1080 * 3 / 4):
                            ...
                            """
                            # 这个到后面需要改一下
                            if lhand_is_detected and cropped_lhand_img is not None:
                                cv2.imshow(lhand_img_name, cropped_lhand_img)
                                if cropped_lhand_depth is not None:
                                    cv2.imshow(lhand_depth_name, cropped_lhand_depth)
                                                    
                            if rhand_is_detected and cropped_rhand_img is not None:
                                cv2.imshow(rhand_img_name, cropped_rhand_img)
                                if cropped_rhand_depth is not None:
                                    cv2.imshow(rhand_depth_name, cropped_rhand_depth)
                    ...
                    for i in range(len(current_pose_list)):   #当前的帧
                        ...
                        if frame <= 99999:
                            if human.human_id == 0:
                                ...
                                draw_cicles_once_only_one(bodys=refine_pred_2d[i], image=img, color=color)
                                cv2.line(img, (int(refine_pred_2d[i][4][0]), int(refine_pred_2d[i][4][1])), (int(refine_pred_2d[i][5][0]), int(refine_pred_2d[i][5][1])), color=(0,255,0), thickness=2)
                                cv2.line(img, (int(refine_pred_2d[i][10][0]), int(refine_pred_2d[i][10][1])), (int(refine_pred_2d[i][11][0]), int(refine_pred_2d[i][11][1])), color=(0,255,0), thickness=2)
                            elif human_id < max_human and human_id != -1: # pose_color(human.human_id, max_human)
                                # color = (25 * human_id, 25 * human_id, 25 * human_id)
                                color = pose_color(human.human_id, max_human)
